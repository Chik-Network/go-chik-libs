...
func TestChikConfig_SetFieldByPath(t *testing.T) {
...
func TestChikConfig_SetFieldByPath_FullObjects(t *testing.T) {
	defaultConfig, err := config.LoadDefaultConfig()
	assert.NoError(t, err)
	// Make assertions about the default state, to ensure the assumed initial values are correct
	assert.Equal(t, uint16(9678), defaultConfig.FullNode.Port)
...
// TestChikConfig_SetFieldByPath_FullObjects Tests that we can pass in and correctly parse a whole section of config
// as json or yaml and that it gets set properly
func TestChikConfig_SetFieldByPath_Lists(t *testing.T) {
	defaultConfig, err := config.LoadDefaultConfig()
	assert.NoError(t, err)
	// Make assertions about the default state, to ensure the assumed initial values are correct
	...
	err = defaultConfig.SetFieldByPath([]string{"seeder", "static_peers"}, `["node-test.chiknetwork.com","node-test-2.chiknetwork.com"]`)
	assert.NoError(t, err)
	assert.Equal(t, []string{"node-test.chiknetwork.com", "node-test-2.chiknetwork.com"}, defaultConfig.Seeder.StaticPeers)

	// Test with the actual type as the data to set
	// First reset
	defaultConfig.Seeder.StaticPeers = []string{}
	assert.Equal(t, []string{}, defaultConfig.Seeder.StaticPeers)
	err = defaultConfig.SetFieldByPath([]string{"seeder", "static_peers"}, []string{"node-test.chiknetwork.com", "node-test-2.chiknetwork.com"})
	assert.NoError(t, err)
	assert.Equal(t, []string{"node-test.chiknetwork.com", "node-test-2.chiknetwork.com"}, defaultConfig.Seeder.StaticPeers)
...
func TestChikConfig_SetFieldByPath_Lists_SingleItems(t *testing.T) {
	defaultConfig, err := config.LoadDefaultConfig()
	assert.NoError(t, err)
	// Make assertions about the default state, to ensure the assumed initial values are correct
	assert.Equal(t, []string{}, defaultConfig.Seeder.StaticPeers)
	...
	err = defaultConfig.SetFieldByPath([]string{"seeder", "static_peers", "0"}, "test-host.chiknetwork.com")
	assert.NoError(t, err)
	assert.Equal(t, []string{"test-host.chiknetwork.com"}, defaultConfig.Seeder.StaticPeers)

	err = defaultConfig.SetFieldByPath([]string{"full_node", "full_node_peers", "0", "host"}, "node-0-override.chiknetwork.com")
	assert.NoError(t, err)
	assert.Equal(t, "node-0-override.chiknetwork.com", defaultConfig.FullNode.FullNodePeers[0].Host)

	defaultConfig.FullNode.FullNodePeers = []config.Peer{{Host: "testnode.example.com", Port: 1234}}
	err = defaultConfig.SetFieldByPath([]string{"full_node", "full_node_peers", "0", "host"}, "node-0-override-2.chiknetwork.com")
	assert.NoError(t, err)
	assert.Equal(t, "node-0-override-2.chiknetwork.com", defaultConfig.FullNode.FullNodePeers[0].Host)
	assert.Equal(t, uint16(1234), defaultConfig.FullNode.FullNodePeers[0].Port)

	err = defaultConfig.SetFieldByPath([]string{"full_node", "full_node_peers", "0", "port"}, "9678")
	assert.NoError(t, err)
	assert.Equal(t, "node-0-override-2.chiknetwork.com", defaultConfig.FullNode.FullNodePeers[0].Host)
	assert.Equal(t, uint16(9678), defaultConfig.FullNode.FullNodePeers[0].Port)
	...
	err = defaultConfig.SetFieldByPath([]string{"full_node", "full_node_peers", "0"}, config.Peer{
		Host: "node-0-override-frompeer.chiknetwork.com",
		Port: 9999,
	})
	assert.NoError(t, err)
	assert.Equal(t, "node-0-override-frompeer.chiknetwork.com", defaultConfig.FullNode.FullNodePeers[0].Host)
	assert.Equal(t, uint16(9999), defaultConfig.FullNode.FullNodePeers[0].Port)
}

func TestChikConfig_FillValuesFromEnvironment(t *testing.T) {
	defaultConfig, err := config.LoadDefaultConfig()
	assert.NoError(t, err)
	// Make assertions about the default state, to ensure the assumed initial values are correct
	assert.Equal(t, uint16(9678), defaultConfig.FullNode.Port)
	...
	assert.NoError(t, os.Setenv("chik.full_node.port", "1234"))
	assert.NoError(t, os.Setenv("chik__full_node__rpc_port", "5678"))
	assert.NoError(t, os.Setenv("chik.network_overrides.constants.mainnet.DIFFICULTY_CONSTANT_FACTOR", "44445555"))
	assert.NoError(t, os.Setenv("chik.selected_network", "unittestnet"))
	assert.NoError(t, os.Setenv("chik__logging__log_level", "INFO"))
...
func TestChikConfig_ParsePathsAndValuesFromStrings(t *testing.T) {
	// A mix of paths with and without prefixes with both separators
	strings := []string{
		"chik.full_node.port=9678",
		"chik__full_node__db_sync=auto",
	...
	assert.Equal(t, "9678", result["full_node.port"].Value)
	...
	assert.Len(t, result, 4) // 4 because it won't strip chik prefix if its found
	assert.Contains(t, result, "chik.full_node.port")
	assert.Equal(t, []string{"chik", "full_node", "port"}, result["chik.full_node.port"].Path)
	assert.Equal(t, "9678", result["chik.full_node.port"].Value)
	assert.Contains(t, result, "chik__full_node__db_sync")
	assert.Equal(t, []string{"chik", "full_node", "db_sync"}, result["chik__full_node__db_sync"].Path)
	assert.Equal(t, "auto", result["chik__full_node__db_sync"].Value)
...
func TestChikConfig_ParsePathsFromStrings(t *testing.T) {
	// A mix of paths with and without prefixes with both separators
	strings := []string{
		"chik.full_node.port",
		"chik__full_node__db_sync",
	...
	assert.Len(t, result, 4) // 4 because it won't strip chik prefix if its found
	assert.Contains(t, result, "chik.full_node.port")
	assert.Equal(t, []string{"chik", "full_node", "port"}, result["chik.full_node.port"])
	assert.Contains(t, result, "chik__full_node__db_sync")
	assert.Equal(t, []string{"chik", "full_node", "db_sync"}, result["chik__full_node__db_sync"])
	assert.Contains(t, result, "full_node.db_readers")
